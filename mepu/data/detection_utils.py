...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if annos[0].get("soft_label", -1) != -1:
        soft_labels = [float(obj["soft_label"]) for obj in annos]
        soft_labels = torch.tensor(soft_labels, dtype=torch.float64)
        target.soft_labels = soft_labels
